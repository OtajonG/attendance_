...
def record_attendance(student_id):
    ...
    execute_db("INSERT INTO attendance (student_id, timestamp) VALUES (?, ?)", (student_id, timestamp))
...
@app.route("/records")
def records():
    attendance_records = query_db("SELECT s.name, a.timestamp FROM attendance a JOIN students s ON a.student_id = s.id") # Using 'name' here
...
def verify_face():
    ...
    if not webcam_face_locations:
        return jsonify({"success": False, "message": "No face detected in the captured image."})

    webcam_face_encoding = face_recognition.face_encodings(webcam_img, webcam_face_locations)[0]

    student_data = query_db("SELECT face_encoding FROM students WHERE id = ?", (student_id,), one=True)

    if student_data and student_data['face_encoding']:
        stored_face_encoding = np.frombuffer(student_data['face_encoding'], dtype=np.float64)
